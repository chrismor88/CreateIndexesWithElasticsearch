...
public class InsertRecordRelationBetweenTypesThread extends Thread {
	...
	public InsertRecordRelationBetweenTypesThread(BlockingQueue<String> inputBuffer, BlockingQueue<String> outputBuffer, WikiminigIndexes index) {
		...
		while(true){
			try {
				...
				if(!message.equals(Message.FINISHED_PRODUCER)){
					...
					index.insertRecordRelationBetweenTypes(fieldsMessage[0],fieldsMessage[1],fieldsMessage[2]);
				}
				else{
